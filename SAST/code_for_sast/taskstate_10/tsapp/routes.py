...
def reg():
    if request.method == "POST":
        ...
        if not(ulogin or upassword):
            ...
            # VULNERABILITY FOUND: CWE-259 Use of Hard-coded Password
...
def passrec():
    ...
    if len(request.args) > 0:
        if request.args.get('login'):
            ...
            # VULNERABILITY FOUND: CWE-532 Information Disclosure Through Logs
...
def taskcreate():
    if request.method == "POST":
        # VULNERABILITY FOUND: CWE-95 Improper Neutralization of Directives in Dynamically Evaluated Code ('Eval Injection')
